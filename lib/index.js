...
function apply(ctx, config) {
  ...
  const { cacheTtlMs, catalogFile, historyFile, historyMaxEntries } = config;
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      if (catInfo !== undefined) {
        try {
          const cached = JSON.parse(await ctx.fs.readText(catPath, exec.signal));
          if (cached && Array.isArray(cached.repos) && cached.syncedAt) {
            const age = Date.now() - new Date(cached.syncedAt).getTime();
            if (age < cacheTtlMs) {
              return { syncedAt: cached.syncedAt, count: cached.count, truncated: !!cached.truncated, note: note() + "（命中缓存）" };
            }
          }
